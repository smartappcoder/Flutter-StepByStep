...
class ContactPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return new Theme(
        data: new ThemeData(
            ...
            primarySwatch:
                _contact.gender == 'male' ? Colors.teal : Colors.pink),
        child: new Scaffold(
  ...
  _ContactCategory _buildPhoneCategory() {
    var phoneItems = _contact.phones
        .map((phone) => new _ContactCategoryItem(
            icon: Icons.message, lines: <String>[phone.number, phone.type]))
        .toList();

    return new _ContactCategory(icon: Icons.call, children: phoneItems);
  }
